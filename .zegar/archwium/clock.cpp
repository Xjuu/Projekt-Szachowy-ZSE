...
void format_time(uint32_t ms, char *out, size_t out_len, TimeFormat format) {
    ...
    if (format == TIME_FORMAT_MM_SS) {
        snprintf(out, out_len, "%02u:%02u", minutes, seconds);
    } else {
        if (minutes > 0) {
            snprintf(out, out_len, "%02u:%02u", minutes, seconds);
        } else {
            snprintf(out, out_len, "%02u.%03u", seconds, milliseconds);
        }
    }
}
...
void switch_side(ClockState *clock_state, ActiveSide pressed_side) {
    ...
    // Dodaj czas dla gracza który właśnie zagrał ruch, zanim przełączymy aktywnego gracza
